    ...
    if (num_free_frames > 0)
    {
        ...
        for (int i = 0; i < num_frames; i++)
        {
            if (frames[i].page_number == -1)
            {
                ...
                frames[i].page_number = page_number; // Assign page to the frame
                frames[i].timestamp = timestamp;     // Set the timestamp
                num_free_frames--;
        ...
        else
        {
            ...
            if (use_lru)
            {
                ...
                for (int i = 0; i < num_frames; i++)
                {
                    ...
                    {
                        frames[i].timestamp = page_references; // Update to the latest timestamp
                        break;
                    }
                }
            }
